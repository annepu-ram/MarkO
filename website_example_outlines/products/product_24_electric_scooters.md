...
3. **layout-row (Feature: Range per Charge)** → `comp_0_components_2`
...
4. **layout-row (Feature: Top Speed & Performance)** → `comp_0_components_3`
...
5. **layout-row (Feature: Motor Power)** → `comp_0_components_4`
...
6. **layout-row (Feature: Battery & Charging Time)** → `comp_0_components_5`
...
7. **layout-row (Feature: Frame & Weight)** → `comp_0_components_6`
...
8. **layout-row (Feature: Braking System)** → `comp_0_components_7`
...
9. **layout-row (Feature: Display & App)** → `comp_0_components_8`
...
10. **layout-row (Feature: Terrain Suitability)** → `comp_0_components_9`
...
11. **layout-row (Feature: Price & Subsidy)** → `comp_0_components_10`
...
12. **layout-row (Feature: Colors & Variants)** → `comp_0_components_11`
...
13. **layout-row (Reviews & Test Ride)** → `comp_0_components_12`
...
3. **layout-row (Range Confidence)** → Heading ("200 km on a Single Charge") + paragraph ("That's your daily commute, 5 days a week, on one charge.") + counter-up (200) + caption ("km Certified Range"). Image (route map visualization showing 200 km coverage area from city center).
4. **layout-row (Cost Savings)** → Heading ("Save Rs 50,000 Every Year") + columnsgrid (2 columns): Column 1 — heading ("Petrol Scooter") + counter-up (60,000) + caption ("Annual Running Cost") + paragraph (fuel + maintenance). Column 2 — heading ("VoltRide Max") + counter-up (10,000) + caption ("Annual Running Cost") + paragraph (electricity + zero maintenance). Badge ("5x Cheaper to Run").
5. **layout-row (Performance Stats)** → columnsgrid (4 columns): counter-up (200) + caption ("km Range"), counter-up (95) + caption ("km/h Top Speed"), counter-up (2.5) + caption ("sec 0-40"), counter-up (0) + caption ("Emissions").
6. **layout-row (Smart Features)** → Carousel: Slide 1 — Dashboard demo. Slide 2 — App features. Slide 3 — Navigation. Slide 4 — Theft protection. Slide 5 — OTA updates.
7. **layout-row (Charging Ecosystem)** → Heading ("Charge Everywhere") + image (charging station map) + columnsgrid (2 columns): Home Charging (time, cost per charge) vs Fast Charging (time, network size). Counter-up (5,000) + caption ("Fast Chargers Nationwide").
8. **layout-row (Color Configurator)** → Heading ("Choose Your Color") + columnsgrid of color swatches + large scooter image that represents selection.
9. **layout-row (Pricing & EMI)** → columnsgrid (2 columns): Column 1 — heading (launch price) + badge ("Launch Offer — Save Rs 10,000") + button ("Reserve Now"). Column 2 — heading ("EMI from Rs 2,499/month") + paragraph (0% down payment options) + button ("Calculate EMI").
10. **layout-row (Early Adopter Testimonials)** → Carousel of beta tester/reviewer quotes + ratings. Counter-up (10,000) + caption ("Pre-Orders").
11. **layout-row (FAQ)** → Accordion: 8 questions on range, charging, warranty, subsidy, servicing, insurance, test rides.
12. **layout-row (Final CTA)** → Heading ("Your City. Your Way. Zero Emissions.") + button ("Book Test Ride") + button ("Reserve Now") + caption ("Fully refundable. Deliveries starting March 2026.").
13. **layout-row (Footer)** → Minimal footer with brand, legal, contact, social.
...
3. **layout-row (Quick Nav by Brand)** → Ticker or columnsgrid (horizontal): Ola, Ather, TVS, Hero Vida, Bajaj Chetak, Revolt, Simple Energy — each with brand logo + flagship model name + "from Rs X".
4. **layout-row (Head-to-Head Comparison)** → Heading ("Compare Top Electric Scooters") + columnsgrid (6 columns): Column 1 (spec labels: Range, Top Speed, Battery, Charging Time, Motor Power, Weight, Price), Columns 2-6 (Ola S1 Pro, Ather 450X, TVS iQube ST, Hero Vida V1, Bajaj Chetak). Badges on "Best Range", "Best Performance", "Best Value".
5. **layout-row (Bestsellers)** → Heading ("Most Popular") + columnsgrid (4 columns): Each — image, badge, heading (model name), rating, paragraph (range + speed + price), button ("View Details").
6. **layout-row (Budget Picks)** → Heading ("Under Rs 1 Lakh") + columnsgrid (3 columns): Budget EV cards with image, name, range, price after subsidy, button.
7. **layout-row (Premium Picks)** → Heading ("Premium — Rs 1.5 Lakh+") + columnsgrid (3 columns): Premium EV cards with image, name, highlight feature, price, button.
8. **layout-row (Subsidy Calculator)** → Heading ("Calculate Your On-Road Price") + form: dropdown (select state), dropdown (select model), heading (calculated on-road price), paragraph (breakdown: ex-showroom - FAME II - state subsidy + registration + insurance). Button ("Get Exact Quote").
9. **layout-row (Charging Infrastructure Map)** → Heading ("Charging Stations Near You") + image (India map with charger density) + paragraph ("Enter your city to find the nearest charging points.") + form (textbox for city + button "Find Chargers").
10. **layout-row (Buying Guide)** → Heading ("EV Scooter Buying Guide") + accordion: Item 1 ("What range do I need for my daily commute?"), Item 2 ("How much does it cost to charge at home?"), Item 3 ("Which state gives the best subsidy?"), Item 4 ("Battery warranty — what to look for?"), Item 5 ("Removable vs fixed battery — pros and cons"), Item 6 ("E-scooter vs petrol scooter — total cost comparison").
11. **layout-row (Customer Reviews)** → Heading ("Owner Reviews") + carousel of review cards with rating, model purchased, range feedback, blockquote.
12. **layout-row (Test Ride Booking)** → Heading ("Book a Free Test Ride") + columnsgrid (2 columns): Column 1 — form (dropdown model, dropdown city, textbox name, textbox phone, button "Book Now"). Column 2 — image (happy rider on test ride) + caption ("Test rides available in 150+ cities").
13. **layout-row (Footer)** → columnsgrid (4 columns): Store info, brands + models, support + policies, contact + social.
