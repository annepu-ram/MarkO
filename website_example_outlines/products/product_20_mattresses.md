...
3. **layout-row (Feature: Material — Foam/Spring/Latex)** → `comp_0_components_2`
...
4. **layout-row (Feature: Firmness Level)** → `comp_0_components_3`
...
5. **layout-row (Feature: Sizes Available)** → `comp_0_components_4`
...
6. **layout-row (Feature: Cooling Technology)** → `comp_0_components_5`
...
7. **layout-row (Feature: Edge Support)** → `comp_0_components_6`
...
8. **layout-row (Feature: Trial Period)** → `comp_0_components_7`
...
9. **layout-row (Feature: Warranty)** → `comp_0_components_8`
...
10. **layout-row (Feature: Price & Payment Plans)** → `comp_0_components_9`
...
11. **layout-row (Feature: Sleep Position Guide)** → `comp_0_components_10`
...
12. **layout-row (Feature: Certifications)** → `comp_0_components_11`
...
14. **layout-row (Complete Your Sleep Setup)** → `comp_0_components_13`
...
3. **layout-row (Pain Point)** → Heading ("68% of Indians Wake Up With Back Pain") + paragraph (poor mattress quality stats, sleep deprivation impact) + counter-up stats: 68% back pain, 4.2 crore affected, Rs 15,000 avg annual medical cost.
4. **layout-row (Technology Reveal)** → Full-width cross-section image with animated layer labels. Heading ("5 Layers. Zero Compromises.") + paragraph (brief on each layer). Badge ("Patented AdaptFoam Technology").
5. **layout-row (Before & After)** → columnsgrid (2 columns): Column 1 — heading ("Regular Mattress") + image (poor spine alignment) + list of problems. Column 2 — heading ("CloudRest Pro") + image (proper spine alignment) + list of benefits. Badges highlighting differences.
6. **layout-row (Celebrity / Expert Endorsement)** → Blockquote from orthopaedic doctor + image (doctor portrait) + caption (name, credentials). Rating (5 stars, "Doctor Recommended").
7. **layout-row (Feature Carousel)** → Carousel: Slide 1 — Cooling tech. Slide 2 — Edge support. Slide 3 — Motion isolation. Slide 4 — Hypoallergenic cover. Slide 5 — Zero partner disturbance.
8. **layout-row (Social Proof Wall)** → Heading ("50,000+ Happy Sleepers") + columnsgrid (3 columns) of review cards with ratings, quotes, reviewer photos.
9. **layout-row (Size & Price Grid)** → Heading ("Choose Your Size") + columnsgrid (4 columns): Each size (Single, Double, Queen, King) with dimensions, price, badge on bestseller, button ("Buy Now").
10. **layout-row (Risk Removal)** → columnsgrid (3 columns): counter-up (100) + "Night Trial", counter-up (10) + "Year Warranty", counter-up (0) + "Risk — Free Returns". Paragraph ("We believe in our mattress so much, we'll let you sleep on it for 100 nights risk-free.").
11. **layout-row (FAQ)** → Accordion: 8 questions covering firmness, delivery, trial, warranty, sizes, EMI, care instructions, certification.
12. **layout-row (Final CTA)** → Heading ("Your Best Sleep Starts Tonight") + button ("Shop CloudRest Pro") + caption ("Free delivery. 100-night trial. 10-year warranty.").
13. **layout-row (Footer)** → Minimal footer with brand, contact, policies, social links.
...
3. **layout-row (Sleep Quiz CTA)** → Centered: heading ("Not Sure Which Mattress?") + paragraph ("Take our 2-minute sleep quiz") + button ("Find My Mattress") + caption ("Personalized recommendation based on your sleep style").
4. **layout-row (Category Quick Nav)** → Ticker or columnsgrid (horizontal): Memory Foam, Pocket Spring, Latex, Orthopaedic, Hybrid, Kids, Guest Room — each with icon + label + "from Rs X,XXX".
5. **layout-row (Filter Bar)** → Layout-row with form elements: dropdown (material type), dropdown (firmness), dropdown (size), dropdown (price range), dropdown (brand). Button ("Apply Filters").
6. **layout-row (Bestsellers)** → Heading ("Our Bestsellers") + columnsgrid (3 columns): Each — image, badge ("Bestseller" / "Editor's Pick"), heading (product name), rating, paragraph (material + firmness + key feature), heading (price + strikethrough), button ("View Details").
7. **layout-row (Comparison Table)** → Heading ("Compare Our Top Mattresses") + columnsgrid (5 columns): Column 1 (feature labels: Material, Firmness, Layers, Cooling, Trial, Warranty, Price), Columns 2-5 (4 mattresses compared). Badges on "Best Value" and "Premium Pick".
8. **layout-row (By Sleep Position)** → Heading ("Shop by Sleep Position") + columnsgrid (3 columns): Side Sleepers (image + recommended mattresses + button), Back Sleepers (image + recommended + button), Stomach Sleepers (image + recommended + button).
9. **layout-row (Budget Picks)** → Heading ("Under Rs 10,000") + columnsgrid (4 columns): Budget mattress cards with image, name, firmness badge, price, rating, button.
10. **layout-row (Premium Collection)** → Heading ("Premium Collection — Rs 25,000+") + columnsgrid (3 columns): Premium cards with image, name, technology highlight, price, rating, button.
11. **layout-row (Buying Guide)** → Heading ("Mattress Buying Guide") + accordion: Item 1 ("Memory Foam vs Spring vs Latex — which is best?"), Item 2 ("How firm should my mattress be?"), Item 3 ("What size do I need?"), Item 4 ("How often should I replace my mattress?"), Item 5 ("Is online mattress buying safe?"), Item 6 ("What certifications should I look for?").
12. **layout-row (Customer Reviews)** → Heading ("What Our Sleepers Say") + carousel of review cards: rating + blockquote + caption (product purchased + verified badge).
13. **layout-row (Bundle Deals)** → Heading ("Sleep Bundles — Save Up to 30%") + columnsgrid (3 columns): Bundle 1 (mattress + 2 pillows), Bundle 2 (mattress + protector + pillows), Bundle 3 (complete bedroom set). Each with original price strikethrough, bundle price, savings badge, button.
14. **layout-row (Newsletter)** → Left column: heading ("Sleep Tips & Exclusive Deals") + form (textbox + button "Subscribe"). Right column: heading ("Need Help?") + paragraph + button ("Chat With Sleep Expert").
15. **layout-row (Footer)** → columnsgrid (4 columns): Brand info, product categories, policies + support, contact + social.
